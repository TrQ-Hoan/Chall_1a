...
$sql = "SELECT COUNT(*) AS total FROM `challenges` JOIN `users` ON `challenges`.`teacherid` = `users`.`id` WHERE
            LOWER(`title`) LIKE ? OR
            LOWER(`hints`) LIKE ? OR 
            LOWER(`users`.`fullname`) LIKE ? OR 
            LOWER(`files`) LIKE ?;";
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        if ($_SESSION['user_role'] === 'teacher') {
                            ...
                            echo '<a href="/remove.php?tbl=challenges&id=' . $row['id'] . '" onclick="return confirm(\'Are you sure you want to delete it?\');" class="btn btn-sm btn-danger me-2"><i class="bi bi-trash"></i></a>';
                        }
